...
public class SunmiPrintHelper {
	...
	public void initSunmiPrinterService(Context context) {
		try {
			boolean success = InnerPrinterManager.getInstance()
		...
		try {
			try {
				sunmiPrinterService.setPrinterStyle(
						...
						enable ? WoyouConsts.ENABLE : WoyouConsts.DISABLE
				);
			} catch (RemoteException e) {
				if (enable) {
	 ...
	 */
	public void printRow(String[] txts, int[] width, int[] align) {
		if (sunmiPrinterService == null) {
			//TODO Service disconnection processing
			return;
		}
	...
	/**
	 * Starts a print transaction with buffer
	 */
